	...
	if(isset($_POST['employee'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				...
				if($query->num_rows > 0){
					$output['error'] = true;
					$output['message'] = 'Ya has registrado tu ingreso hoy';
				}
				else{
					//updates
					...
					$horaInicial=$srow['time_in'];
					$minutoAnadir=15;
					$segundos_horaInicial=strtotime($horaInicial);
					$segundos_minutoAnadir=$minutoAnadir*60;
					$nuevaHora=date("H:i",$segundos_horaInicial+$segundos_minutoAnadir);
					$logstatus = ($lognow > $nuevaHora) ? 0 : 1;
					...
					//
					if ($logstatus == 1) {
						$sql = "INSERT INTO attendance (employee_id, date, time_in, status, month) VALUES ('$id', '$date_now', NOW(), '$logstatus', '$monthName')";
						if($conn->query($sql)){
						...
						$output['message'] = 'No se registrará la asistencia, más de 15 minutos tarde';
					}
				}
			}
			else{
				...
				if($query->num_rows < 1){
					$output['error'] = true;
					$output['message'] = 'No ha registrado la entrada. No puede registrar la salida';
				}
				else{
					$row = $query->fetch_assoc();
					if($row['time_out'] != '00:00:00'){
						$output['error'] = true;
						$output['message'] = 'Ya has registrado tu salida hoy';
					}
					else{
